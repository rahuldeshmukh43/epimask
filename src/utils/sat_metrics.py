...
from epimask.external.satdepth.src.utils.SatCV import relative_pose_error, refine_affine_fundamental_matrix
...
def distance_errors(pts0, pts1, F_gt):
    ...
    """
    pts0 = convert_points_to_homogeneous(pts0)  #[N, 3]
    ...
    d1 = p1Fp0**2 /(Fp0[:, 0]**2 + Fp0[:, 1]**2) # [N]
    d2 = p1Fp0**2 /(Ftp1[:, 0]**2 + Ftp1[:, 1]**2)  # [N]
    d_symm = d1 + d2 # squared
    # d_symm = 0.5 * (torch.sqrt(d1) + torch.sqrt(d2)) #[N] not squared
...
def aggregate_metrics(metrics, epi_err_thr=5e-4):
    ...
    precs = epidist_prec(np.array(metrics['epi_errs'], dtype=object)[unq_ids],
                         dist_thresholds, ret_dict=True, return_counts=True)  # (prec@err_thr)

    return {**aucs, **scale_aucs, **precs}

def epidist_prec(errors, thresholds, ret_dict=False, return_counts=False):
    results = {}
    for thr in thresholds:
        prec_list, count_list = [], []
        for errs in errors:  # errs is a 1D array of distances folr one image pair
            correct_mask = errs < thr
            num_correct = int(np.sum(correct_mask))
            count_list.append(num_correct)
            prec_list.append(np.mean(correct_mask) if len(correct_mask) > 0 else 0)
        mean_prec = np.mean(prec_list) if len(prec_list) > 0 else 0
        mean_count = np.mean(count_list) if len(count_list) > 0 else 0  # avg correct matches per pair

        if ret_dict:
            results[f'prec@{thr:.0e}'] = mean_prec
            if return_counts:
                results[f'num_correct@{thr:.0e}'] = mean_count
        else:
            if return_counts:
                results[thr] = (mean_prec, mean_count)
            else:
                results[thr] = mean_prec
    return results
