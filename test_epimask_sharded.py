...
from epimask.src.datasets.satdepth_webdataset import ShardedSatDepthDataset_Test_Whole_Image
...
def extract_matches(model, 
    ...
    if args.dataset_type == "default":
        ...
        # read shard_size_json
        with open(os.path.join(args.shard_dir, args.shard_size_json), 'r') as f:
            shard_size_json_data = json.load(f)
        ...
        # get shard size
        _pair_name = "%s-%s_and_%s"%(aoi_name, img0_base, img1_base)
        shard_size = shard_size_json_data[_pair_name]
        this_pair_shard_dir = os.path.join(args.shard_dir, _pair_name)
        logger.info("Num grid patches: %d"%(shard_size))

        test_dataset = ShardedSatDepthDataset_Test_Whole_Image(
                shard_dir= this_pair_shard_dir,
...
def parse_args():
    ...
    parser.add_argument("--shard_dir", type=str, default="", help="path to top dir for all whole image shards")
    parser.add_argument("--shard_size_json", type=str, default="all_pairs_num_samples.json", help="json indicating num samples in each shard")
